...
using FlightProjectDBGenerator.Properties;
    ...
    class MainWindowViewModel : INotifyPropertyChanged
    {
        ...
        public bool IsProcessRunning
        {
            ...
            set
            {
                ...
                OnPropertyChanged("IsProcessRunning");
            }
        }
        public bool IsErrorUp
        {
            get
            {
                return _isErrorUp;
        ...
        private void TicketDBCreator()
        {
            while (true)
            {
                if (TicketsPerCustomerDataSet.runThread)
                {
                    ...
                    while (continueFlag)
                    {
                        if (tickets.Count == TicketsPerCustomerDataSet.listSize)
                        {
                            int totalIndex = 0;
                            for (int keyIndex = 0; keyIndex < CustomerDataSet.listSize; keyIndex++)
                            {
                            ...
                            GetListBoxIndex($"Tickets Registered to Database.");
        ...
        private void ErrorChecker()
        {
            if (TicketsPerCustomerDataSet.IsRandomDisabled)
            {
                if (FlightsPerCompanyDataSet.IsRandomDisabled)
                {
                    ...
                    {
                        Application.Current.Resources["ErrorBrush"] = new SolidColorBrush(Colors.Red);
                        IsErrorUp = true;
                    }
                    else
                    {
                        Application.Current.Resources["ErrorBrush"] = new SolidColorBrush(Colors.Gray);
                        IsErrorUp = false;
                    }
                }
                else
